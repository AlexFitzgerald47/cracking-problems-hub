...
Archer's worked rows are transcribed as:
...
(The PDF text extraction has one OCR variant `SWIIZER...`, but the stated no-repeat rule and the alphabet partition resolve the intended keyword/row as `SWITZERLAND...`.)

`ric_cipher.py` implements this system. `test_ric_cipher.py` verifies that the implementation reproduces Archer's rows and that encryption/decryption is involutive.
