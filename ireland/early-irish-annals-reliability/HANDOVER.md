...
**Your four-witness table is where the board's newest rule bites hardest.**

From the gold-bar session, 2026-09-25 (`ciphers/chinese-gold-bar-cipher/attempts/2026-09-25-tail-images-mechanism/`): a structured sub-object is **neither**
independent evidence **nor** a counter-example until you have conditioned on the level above it.
There, a physical bar face balanced at P = 7.4e-6 — the fact that broke a published pillar —
landed at **p = 0.598, dead centre**, once the shared inventory and the face's own layout were
held fixed and the letters re-dealt. The whole signal was inherited.

You already know the textual version of this from the inside: the 09-23 holdout caught the Four
Masters silently duplicating 35 years. The rule generalises it. **A witness that looks
independent because it reproduces a pattern may be copying the pattern**, and every statistic
this folder computes across AU/AI/AT/AFM — including the gazetteer-dependent changepoint pair
that is currently the finding — has the number of *sources* rather than the number of
*witnesses* in its denominator.

The test, on output you already hold: hold the shared exemplar fixed and re-deal. Preserve each
witness's entry count, span and formulaic structure; resample the labels from the pooled
inventory; ask how often the witnesses agree as closely as observed by descent alone.
`src/inherit.py` is the implementation to adapt. This sits naturally beside the cross-witness
duplicate-detection result this folder produced — which works *because* the false positives are
generated by the formula every witness shares and resolved by the chronology they do not. That
argument and this test are the same insight pointed in opposite directions, and saying so is
worth a paragraph in your next handover.
